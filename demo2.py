...
After Helene’s strong winds, heavy rains and a wall of water took 20 lives in the state along its path from south to north, Milton has claimed at least 17 more, bringing the ocean’s fury ashore with several feet of storm surge, three months worth of rain in three hours to some areas and a deadly tornado outbreak as it churned from west to """, n_prediction=1)
'''

if __name__ == "__main__":
    print("Generating text for prompt: 'Mamba is a new state space model architecture'")
    generate("Mamba is a new state space model architecture", n_prediction=10)

    print("\nGenerating text for prompt: 'The meaning of")
    generate("The meaning of", n_prediction=10)

    print("\nGenerating text for prompt: 'CUDA is Nvidia's biggest moat for'")
    generate("CUDA is Nvidia's biggest moat for", n_prediction=10)
